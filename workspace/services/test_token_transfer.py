...
def test_wallet_creation():
    ...
    try:
        ...
        assert wallet.entity_id == entity_id
...
def test_token_transfer():
    ...
    receiver_id = "receiver-entity"
    
    try:
        # 送信者・受信者ウォレット作成
        sender = create_wallet(sender_id, initial_balance=1000.0)
        receiver = create_wallet(receiver_id, initial_balance=100.0)
        ...
        # クリーンアップを保証
        delete_wallet(sender_id)
        delete_wallet(receiver_id)


def test_insufficient_balance():
    """残高不足テスト"""
    print("\n=== Insufficient Balance Test ===")
    sender_id = "poor-entity"
    receiver_id = "rich-entity"
    
    try:
        sender = create_wallet(sender_id, initial_balance=10.0)
        receiver = create_wallet(receiver_id, initial_balance=100.0)
...
def test_invalid_transfer():
    ...
    sender_id = "sender-invalid"
    receiver_id = "receiver-invalid"
    
    try:
        sender = create_wallet(sender_id, initial_balance=100.0)
        receiver = create_wallet(receiver_id, initial_balance=100.0)
...
def test_multiple_transfers():
    ...
    try:
        alice = create_wallet(alice_id, initial_balance=1000.0)
        bob = create_wallet(bob_id, initial_balance=0.0)
        charlie = create_wallet(charlie_id, initial_balance=0.0)
