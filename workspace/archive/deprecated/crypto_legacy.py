...
class MessageType:
    """Message type constants for protocol v1.0"""
    HANDSHAKE = "handshake"
...
@dataclass
class SecureSession:
    """Secure session state for protocol v1.0"""
    session_id: str
    sender_id: str
    recipient_id: str
    sequence_num: int = 0
...
@dataclass
class MessageChunk:
    """Message chunk for large payload transfer"""
    chunk_id: str
    session_id: str
    message_id: str
    sequence_num: int
...
class SessionManager:
    """
    Session management for peer communication
    - Session ID (UUID)
    - Sequence number management (per-session, increments from 1)
    ...
    def __init__(self):
        self._sessions: Dict[str, SessionManager.SessionInfo] = {}
        self._peer_sessions: Dict[str, str] = {}  # peer_id -> session_id
    
    def create_session(self, peer_id: Optional[str] = None) -> str:
        """
        Create a new session
        
        Args:
            peer_id: Optional peer identifier for this session
    ...
    def get_session_by_peer(self, peer_id: str) -> Optional[SessionInfo]:
        """Get session info by peer ID"""
        session_id = self._peer_sessions.get(peer_id)
        if session_id:
            return self._sessions.get(session_id)
        return None
    
    def get_next_sequence_num(self, session_id: str) -> Tuple[int, bool]:
        """
        Get and increment the next sequence number for a session
        
        Args:
            session_id: Session ID
            
        Returns:
            Tuple of (sequence_number, is_valid)
        """
        ...
        session.sequence_num += 1
        session.last_activity = datetime.now(timezone.utc)
        
        return session.sequence_num, True
    
    def validate_sequence(self, session_id: str, expected_seq: int) -> Tuple[bool, Optional[str]]:
        """
        Validate that the sequence number matches expected value
        ...
        if session.sequence_num + 1 != expected_seq:
            return False, f"Sequence mismatch: expected {session.sequence_num + 1}, got {expected_seq}"
        
        session.sequence_num = expected_seq
        session.last_activity = datetime.now(timezone.utc)
    ...
    def is_expired(self, session_id: str) -> bool:
        ...
        now = datetime.now(timezone.utc)
        expiry = timedelta(minutes=self.SESSION_EXPIRY_MINUTES)
        
        return (now - session.last_activity) > expiry
    
    def update_activity(self, session_id: str) -> bool:
        """Update last activity timestamp"""
        session = self._sessions.get(session_id)
        if session and not self.is_expired(session_id):
            session.last_activity = datetime.now(timezone.utc)
            return True
        return False
    ...
    def close_session(self, session_id: str) -> bool:
        ...
        session = self._sessions.get(session_id)
        if session:
            if session.peer_id and session.peer_id in self._peer_sessions:
...
class WalletManager:
    ...
    Note: このクラスはcryptographyライブラリを使用し、crypto.pyの他の機能とは独立して動作する。
    """
    ...
    def _generate_keypair_cryptography(self) -> Tuple[str, str]:
        """cryptographyを使用してEd25519鍵ペアを生成
