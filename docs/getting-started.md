...
# プロファイルを指定
open-entity chat --profile development

# モデルを指定
open-entity chat --model gemini-2.0-flash
```

### 単発実行

```bash
open-entity run "このディレクトリの構造を説明してください"
```
...
# セキュリティプロファイルで起動
open-entity chat --profile security
```
...
```bash
# セッション一覧
open-entity sessions

# 特定のセッションを継続
open-entity chat --session <session-id>
```
