...
def _find_all_profiles_dirs() -> list:
    """全てのプロファイルディレクトリを探す（複数箇所から読み込み用）
    
    優先順位の高い順にリストを返す。同名のエージェント/スキルは
    リストの先頭（優先度高）のものが使われる。
    """
    dirs = []
    ...
    if working_dir:
        wd_profiles = os.path.join(working_dir, "profiles")
        if os.path.exists(wd_profiles) and wd_profiles not in dirs:
            dirs.append(wd_profiles)
    
    # 3. カレントディレクトリ
    cwd_dir = os.path.join(os.getcwd(), "profiles")
    if os.path.exists(cwd_dir) and cwd_dir not in dirs:
        dirs.append(cwd_dir)
    
    # 4. パッケージ内（常に含める）
    pkg_dir = os.path.join(_MOCO_ROOT, "profiles")
    if os.path.exists(pkg_dir) and pkg_dir not in dirs:
        dirs.append(pkg_dir)
...
class AgentLoader:
    def __init__(self, profile: str = "default"):
        ...
        self._refresh_agents_dirs()

    def _refresh_agents_dirs(self) -> None:
        """Recalculate agents directories for the current profile.

        Note: profile can be changed at runtime (e.g. via CLI /profile),
        so this must not be treated as immutable state.
        
        Loads from all available profile directories (cwd, package, etc.)
        """
        all_profiles_dirs = _find_all_profiles_dirs()
        self.agents_dirs = [
            os.path.join(profiles_dir, self.profile, "agents")
            for profiles_dir in all_profiles_dirs
        ]
        # フィルタリング：存在するディレクトリのみ
        self.agents_dirs = [d for d in self.agents_dirs if os.path.isdir(d)]
        
        # 後方互換性のため、最初のディレクトリを agents_dir として保持
        self.agents_dir = self.agents_dirs[0] if self.agents_dirs else ""
    ...
    def load_agents(self) -> Dict[str, AgentConfig]:
        ...
        # 全てのディレクトリからエージェントを読み込む（逆順で処理し、優先度高が上書き）
        for agents_dir in reversed(self.agents_dirs):
            # Support both .md and .yaml files
            for ext in ["*.md", "*.yaml", "*.yml"]:
                search_path = os.path.join(agents_dir, ext)
